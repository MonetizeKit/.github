...
- `scripts/phase/export-secrets.mjs` — runtime credential fetch for the reusable workflows: with the org secret `PHASE_SDLC_TOKEN` (a Phase.dev service token scoped to the `sdlc` environment) it exports only the keys a job asks for, masks them and writes them to `GITHUB_ENV`. Replaces the Phase → GitHub Actions secret sync for automation credentials (the sync is capped at 100 secrets per repository and silently drops the rest; fleet repositories had none). Without the token it prints a notice and the caller's own secrets are used.
